...
namespace AdventCode2019.Day11
{
    public class Worker : IWorker
    {
        public object Work1()
        {
            ...
            var robot = new Robot(new IntCodeComp(array), 0);
            robot.Run();

            return robot.VisitedFields.Keys.Count();
        }
        ...
        public object Work2()
        {
            ...
            var robot = new Robot(new IntCodeComp(array), 1);
            robot.Run();

            int maxX = robot.VisitedFields.Keys.Select(coord => coord.X).Max();
            int minX = robot.VisitedFields.Keys.Select(coord => coord.X).Min();
            int maxY = robot.VisitedFields.Keys.Select(coord => coord.Y).Max();
            int minY = robot.VisitedFields.Keys.Select(coord => coord.Y).Min();
            ...
            foreach (var field in robot.VisitedFields.Where(vf => vf.Value == 1))
            {
                canvas[field.Key.Y - minY, field.Key.X - minX] = '#';
            }
