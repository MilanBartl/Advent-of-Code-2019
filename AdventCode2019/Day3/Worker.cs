...
namespace AdventCode2019.Day3
{
    public class Worker : IWorker
    {
        public object Work1()
        {
            var wires = LoadData(input);

            var intersections = wires[0].Coordinates.Where(c => wires[1].Coordinates.Contains(c)).ToList();
            var center = new Coordinate(0, 0);

            var distances = new List<int>();
            foreach (var intersection in intersections)
            {
                distances.Add(Utilities.CalculateManhattanDistance(center, intersection));
            }
        ...
        public object Work2()
        {
            ...
            foreach (var intersection in intersections)
            {
                distances.Add(wires[0].GetRouteTo(intersection) + wires[1].GetRouteTo(intersection));
            }

            return distances.Min();
        }

        private List<Wire> LoadData(string input)
        {
            var wiresData = input.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            var wires = new List<Wire>();

            foreach (var wireData in wiresData)
            {
                var splits = wireData.Split(',');
                var wire = new Wire();
                ...
                foreach (var split in splits)
                {
                    ...
                    int length = int.Parse(split.Substring(1));
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        {
                            case 'U':
                                actual = new Coordinate(actual.X, actual.Y + 1);
                                break;
                            case 'D':
                                actual = new Coordinate(actual.X, actual.Y - 1);
                                break;
                            case 'L':
                                actual = new Coordinate(actual.X - 1, actual.Y);
                                break;
                            case 'R':
                                actual = new Coordinate(actual.X + 1, actual.Y);
                                break;
                            default:
                                throw new ArgumentException($"Unknown direction of '{direction}'.");
                        }

                        wire.Coordinates.Add(actual);
                        wire.Route.AddLast(actual);
                    }
                }
