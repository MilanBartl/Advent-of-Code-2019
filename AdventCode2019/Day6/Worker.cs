...
namespace AdventCode2019.Day6
{
    public class Worker : IWorker
    {
        public object Work1()
        {
            var orbitObjects = LoadData();

            var com = orbitObjects.First(x => x.Name == "COM");

            return com.Satellites[0].CountOrbits(1);
        }
        ...
        public object Work2()
        {
            ...
            var you = orbitObjects.First(x => x.Name == "YOU");

            int hops = 0;
            var act = you.Parent;
            while (!act.Satellites.Any(sat => sat.Name == "SAN"))
            {
                if (act.HasChild("SAN"))
                    act = act.Satellites.First(x => x.HasChild("SAN"));
                else
                    act = act.Parent;
...
K)L";
        ...
        private const string input = @"RN2)HSX
TYQ)JFL
PBD)P1X
69L)R17
LWG)VPQ
